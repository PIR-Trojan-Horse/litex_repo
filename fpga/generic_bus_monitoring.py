...
def main():
    ...
    alert_led = platform.request("user_led", 0)  # LED de sortie
    counter = Signal(26)  # Compteur de 26 bits pour créer un délai
    led_state = Signal()  # Signal pour contrôler l'état de la LED

    # Logique pour clignoter la LED en utilisant le bit du compteur
    soc.sync += [
        # Incrémenter le compteur à chaque cycle d'horloge
        counter.eq(counter + 1),

        # Si soc.bus_monitor.alert est à 1, on fait clignoter la LED
        If(soc.bus_monitor.alert,
            led_state.eq(counter[25])  # Utiliser le bit 25 du compteur pour alterner la LED
        )
    ]

    # Connexion de la LED à l'état de la LED
