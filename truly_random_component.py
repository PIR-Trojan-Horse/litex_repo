...
class UARTSpy(Module):
    def __init__(self, bus):
        ...
        self.ready = Signal(reset=0)
        self.addr = Signal(32)
        self.index = Signal(10)  # 10 bits pour monter jusqu'à 1024 (unused)
        self.data = Signal(32)
        self.debug_write_data = Signal(32)
        self.debug_write_enable = Signal()
        ...
        self.key = 0xffff0000#0xA5A5A5A5 

        self.submodules.fsm = FSM(reset_state="IDLE")
        self.time_counter = Signal(32,reset=0)
        ...
        self.fsm.act("READ_RAM",
            self.bus.adr.eq(self.addr), # [2:] (<=> (self.addr >> 2)) ## Note : Signal()[0] == LSB and Signal()[-1] = MSB
            self.bus.we.eq(0),
            self.bus.stb.eq(1),
            self.bus.cyc.eq(1),
            If(self.bus.ack,
        ...
        self.fsm.act("WAIT_BEFORE_RESCAN",
            ...
            self.bus.we.eq(0),
            self.debug_write_enable.eq(0),
            If(self.time_counter >= 30,  # longue pause before retry
                NextValue(self.time_counter, 0),
                NextState("READ_RAM")
            ).Else(
                NextValue(self.time_counter, self.time_counter + 1)
...
def tb(dut):
    ...
    while True:
        ...
        ad = (yield dut.uart_master.adr)
        da = (yield dut.get_module("uart_spy").read_data)
        st = (yield dut.get_module("uart_spy").fsm.state)
        if ad != 0:
            print(f"{list(dut.get_module("uart_spy").fsm.actions.keys())[st]}: {da:08x} at {ad:08x}")
