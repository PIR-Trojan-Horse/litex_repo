...
class BusIntegrityMonitor(Module):
    def __init__(self, uart_spy):
        self.alert = Signal()
        ...
        self.sync += [
            If(uart_spy.uart_master_status,
                If((uart_spy.bus.stb & uart_spy.bus.cyc & ~uart_spy.bus.we),  # A read operation
                    If((uart_spy.addr & 0xFF000000) == 0x20000000,  # Only RAM area (0x20000000 region)
                ...
                # Reset monitor when UART is not master
...
class DualMasterSoC(SoCCore):
    def __init__(self, platform, simulate=False):
        ...
        self.submodules.bus_monitor = BusIntegrityMonitor(self.uart_spy)
...
def tb(dut):
    ...
    while True:
        ...
        if current_alert and not last_alert:
            print(f"{RED}⚠️  ALERT: Suspicious UART activity detected! Possible trojan active!")
