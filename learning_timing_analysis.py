...
from analysis_generic_aux import LegitTrafficGenerator, TimerNoise, PRNGComponent
...
class LearningTimingAnalysis(Module):
    def __init__(self, arbiter_bus,name : str):
        ...
        self.latched_alert = Signal(reset=0)
        ...
        self.sync += [
            If(self.alert,
                self.latched_alert.eq(1),
                self.nb_detections.eq(self.nb_detections + 1),
            ).Elif(self.learning,
                self.latched_alert.eq(0)
            ),

            # self.alert.eq( ~self.learning & ( ### DO THE SAME THAT COMBINATORY VERSION
            # ((self.read_counter > self.maximum) & self.reading) | 
            # ((self.read_counter < self.minimum) & ~self.reading))),
            #If(~self.learning & ((self.read_counter > self.maximum) & self.reading),
            #    Display(f"{name} ⚠️ ALERT: read_counter = %d > max = %d", self.read_counter, self.maximum)
            #),
            #If(~self.learning & ((self.read_counter < self.minimum) & ~self.reading),
            #    Display(f"{name} ⚠️ ALERT: read_counter = %d < min = %d", self.read_counter, self.minimum)
            #),
    ...
    def __init__(self, arbiter_bus, cooldown = 10, max_lawfullness = 25):
        ...
        self.sync += [
            ...
            If(self.state == IDLE,
                ...
                self.uart_master_status.eq(1),
                self.nb_activation.eq(self.nb_activation+1),
...
class DualMasterSoC(SoCCore):
    def __init__(self, platform, simulate=False):
        ...
        self.submodules.legit_traffic = LegitTrafficGenerator(self.legit_master,prng=self.prng,base_addr=0x20000000,span=0x1000,access_interval=5)
        self.submodules.bus_legit_traffic = LearningTimingAnalysis(self.legit_master,"LEGIT")
...
def tb(dut):
    ...
    yield dut.bus_legit_traffic.learning.eq(1)
    ...
    min3 = (yield dut.bus_legit_traffic.minimum)
    max3 = (yield dut.bus_legit_traffic.maximum)

    print(f"{RESET}Learnt thresholds: \n - AES:  [{min2},{max2}] \n - UART: [{min1},{max1}] \n - LEGIT: [{min3},{max3}]")

    # PHASE 2 : Detection
    print("Detection phase...")
    yield dut.bus_learning_aes.learning.eq(0)
    yield dut.bus_learning_uart.learning.eq(0)
    yield dut.bus_legit_traffic.learning.eq(0)
    ...
    while time() - start_time < 25:
        ...
        current_alert_3 = yield dut.bus_legit_traffic.alert
        if current_alert_3 and not last_alert_3:
            current_counter = yield dut.bus_legit_traffic.read_counter
            min_val = yield dut.bus_legit_traffic.minimum
            max_val = yield dut.bus_legit_traffic.maximum
            nb_detections += 1
            print(f"{RED}⚠️  ALERT: Suspicious activity detected on LEGIT! ({current_counter} reads > max = {max_val}){RESET}")
        last_alert_3 = current_alert_3
        ...
        yield
    nb_activation = (yield dut.uart_spy.nb_activation)
    

    print(f"{PURPLE}\n=== Detection Statistics ===")
    print(f"Nb activations:  {nb_activation}")
    print(f"Nb detections:  {nb_detections}")
    print(f"Precision:  {(nb_detections - nb_activation) / nb_detections}")
